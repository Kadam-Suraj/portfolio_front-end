    ...
    return (
        <div>
            <div className='border-t-2 transition-all duration-700 select-none cursor-pointer'>
                {tools.map((item, idx) => (
                    <div key={idx}>
                        <div className='relative w-full border-b-2 flex font-semibold py-5 items-center justify-between text-2xl bg-gradient-to-r hover:from-[#B16CEA] hover:via-[#FF5E69] hover:to-[#FFA84B] px-4 group'>
                            <h2>{item.name}</h2>
                            <MdOutlineArrowOutward className='md:block hidden group-hover:animate-pulse' />
                            <Motion
                                initial={{ y: 100, scale: .6, }}
                                whileInView={{ y: 0, scale: 1, }}
                                className='hidden md:group-hover:flex z-10 absolute w-60 -top-[70%] right-[25%] bg-background rounded-xl border aspect-square p-2'
                            >
                                <Image
